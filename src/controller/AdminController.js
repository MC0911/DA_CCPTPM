...
// Sửa thông tin của một tầng
let editFloor = async (floorId, newName) => {
    try {
        await pool.query("UPDATE floors SET name = ? WHERE id = ?", [newName, floorId]);
    } catch (error) {
...
let editTablePage = async (req, res) => {
    try {
        ...
        let tables = await pool.query("SELECT * FROM tables WHERE id = ?", [id]);
        ...
        res.render("edit_table.ejs", { tables: tables[0], floors: floors[0] });
    } catch (error) {
        console.error("Error fetching data: ", error);
